...
\begin{tabular}{|c|c|}
	...
	\hline
	UPDATE	& apre un file per la modifica, se non lo trova lo crea \\
	\hline
\end{tabular}
\end{table}
Per dire a root che si vuole utilizzare un certo TFile come output è necessario eseguire questa funzione:
\begin{lstlisting}[language=C++,label={cod1},mathescape=true,breaklines=true]
	file->cd();
\end{lstlisting}
Da questa linea in poi sarà possibile scrivere nel TFile "file".\\
I vari oggetti di root hanno una funzione chiamata Write, sarà quindi sufficiente, solo dopo aver creato il file di output, chiamare ad esempio:
\begin{lstlisting}[language=C++,label={cod1},mathescape=true,breaklines=true]
	hist->Write();
	canvas->Write();
\end{lstlisting}
Ora si capisce perchè è necessario il comando cd(), qualora siano presenti più TFile nello stesso sorgente il comando Write() sono sa in quale di questi scrivere. Utilizzando il comando cd() da lì in poi Write scriverà nel TFile selezionato fino a nuovo ordine.\\
